...
//		window.location.hash=encodeURIComponent("!sq_start="+JSON.stringify(seach_check_LL)+"&sq_end=");
		...
		find_num_val_.text('');
		$.getJSON('https://secure.cctvplus.com/searchInterface/archive.do?callback=?',
			{
				'pageSize':pagesize,
				'pageNo':curpage,
				'searchTitle':encodeURIComponent(decodeURIComponent(seach_check_LL.keyword_)),
				'isstime':seach_check_LL.DATE_,
				'searchStartTime':seach_check_LL.startTime_,
				'searchEndTime':seach_check_LL.endTime_,'CATEGORY':seach_check_LL.CATEGORY_,'LOCATIONS':seach_check_LL.LOCATIONS_,'RESOLUTION':seach_check_LL.RESOLUTION_,'FENBIANLV':seach_check_LL.FENBIANLV_,'SOURCE':seach_check_LL.SOURCE_
			},function(data){
//				console.log(data);
...
//				textArr.push('<a onselectstart="return false;" onclick="search_fun_LL(',1+curpage,')" class="next">Next</a>');
	...
	function search_load_fun(curpage,type_){
		var video_list_box = $("#video_list_main");
		window.location.hash=encodeURIComponent("!sq_start="+JSON.stringify(seach_check_LL)+"&sq_end=");
		var pagesize = 20;
		$.getJSON('https://secure.cctvplus.com/searchInterface/archive.do?callback=?',
			{
				...
				'isstime':seach_check_LL.DATE_,
				'searchStartTime':seach_check_LL.startTime_,
				'searchEndTime':seach_check_LL.endTime_,
				'CATEGORY':seach_check_LL.CATEGORY_,
				'LOCATIONS':seach_check_LL.LOCATIONS_,
				'RESOLUTION':seach_check_LL.RESOLUTION_,
				'FENBIANLV':seach_check_LL.FENBIANLV_,
				'SOURCE':seach_check_LL.SOURCE_
			},
			function(data){
				console.log(data);
				...
				if(!data || !data.categorys || !data.categorys.length || data.categorys.length == 0){
					video_list_box.html('<p style="text-align:center; padding:10px;">No data</p>');
					return;
				}
				...
				for(var i = 0, len = cate_.length; i<len; i++){
					...
					html_arr.push('<li dcid="',cate_[i].dcid,'" mid="',cate_[i].mid,'"><a href="',cate_[i].detailBuyPath,'">');
					html_arr.push('<dl><dt class="late_img"><img src="',cate_[i].spic,'" /></dt>');
					html_arr.push('<dd><p>',cate_[i].pubtitle,'</p>');
					html_arr.push('<span>',cate_[i].scriptIssTime,'</span></dd></dl></a></li>');
				}
				if( type_ == 'more' ){ //翻页效果append
					video_list_box.append(html_arr.join(''));
				}else{
					video_list_box.html(html_arr.join(''));
				}
				changeImgSize();
			}
		);
...
//		$('#subsearch_val_s').val(decodeURIComponent(seach_check_LL['keyword_']));
		seach_check_LL['keyword_'] = '';//清除之前搜索的关键字
	...
	function change_time_check_(){
		
		var s_time_ = $.trim($('#startTime_val_').val());
		var e_time_ = $.trim($('#endTime_val_').val());
		var re = /^\d{4}-\d{2}-\d{2}$/;
		
		if( s_time_ && e_time_ ){
			if(re.test(s_time_) && re.test(e_time_) && (new Date(e_time_.replace(/-/g,'/'))).getTime() > (new Date(s_time_.replace(/-/g,'/'))).getTime()){
				seach_check_LL['startTime_'] = s_time_;
				seach_check_LL['endTime_'] = e_time_;
				search_load_fun(1);
			}else{
				cusmot_alert_fun("time is error");
			}
...
//			cusmot_alert_fun('Please select at least one Locations');
	...
	$('#startTime_val_, #endTime_val_').focus(function(){
		seach_check_LL['DATE_'] ='';
		$('#ser_data_box input[name="date_radio"][class="data_select"]').iCheck('check');
		change_time_check_();
	});
	
	$(".archive_nav_btn").click(function(){
		if( $(this).hasClass('on') ){
			$(".more_aer_s").hide();
			$(this).removeClass('on');
		}else{
	...
	$('#subsearch_val_box_btn').click(function(e){ //ä¸»æç´¢æé® -æ®éæç´¢
		var ss = $(".custom_sel_text_s").attr('ord_id');
		if($(".custom_sel_text_s").attr('ord_id')=='ord11111'){			
		...
		}else if($(".custom_sel_text_s").attr('ord_id')=='ord44444'){
			window.location.href="/archive/allCollections.shtml#!keyword="+encodeURIComponent($.trim((($('#subsearch_val_s').val()==$('#subsearch_val_s').prop('defaultValue'))?'':$('#subsearch_val_s').val())));
		}else{
