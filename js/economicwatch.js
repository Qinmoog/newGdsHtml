$(function(){
	...
	function search_fun_LL(curpage){ //return false;
		...
		var pagesize = 15;//æ¯é¡µå±ç¤ºæ¡æ°
		
		search_list_box.html('<p style="text-align:center; padding:10px;">Loading...</p>');
		detail_box_.hide();
		search_list_pagebox.html('');
		//'pageSize':pagesize,'pageNo':curpage,'searchTitle':seach_check_LL.keyword_,'isstime':seach_check_LL.DATE_,'searchStartTime':seach_check_LL.startTime_,'searchEndTime':seach_check_LL.endTime_,'CATEGORY':seach_check_LL.CATEGORY_,'LOCATIONS':seach_check_LL.LOCATIONS_,'LANGUAGE':seach_check_LL.LANGUAGE_
		...
		//$.getJSON('https://secure.cctvplus.com/searchInterface/economicWatch.do?callback=?',{'pageSize':pagesize,'pageNo':curpage,'tid':this_tid_},function(data){
		//$.getJSON('http://172.20.1.9:8890/searchInterface/castest.do?callback=?',{'pageSize':pagesize,'pageNo':curpage,'searchTitle':encodeURIComponent(decodeURIComponent(seach_check_LL.keyword_)),'isstime':seach_check_LL.DATE_,'CATEGORY':seach_check_LL.CATEGORY_,'LOCATIONS':seach_check_LL.LOCATIONS_,'LANGUAGE':seach_check_LL.LANGUAGE_},function(data){
...
//		var vid_box_ = $('#video_box'),
//		vid_f_bod_ = $('#data_success_box'),
...
////		$('#ckplayer_a1').remove();
			...
			if(d && d.items && d.items.length){
				...
				$.each(items_,function(i,obj){
					...
					if(mtype == 'T2'){
						...
						sharebtns_.push('<a class="btn01"></a><input type="hidden" value="http://www.facebook.com/share.php?u=',obj.detailBuyPath,'&t=',obj.slug,'" id="facebook_val">');
						sharebtns_.push('<a rel="nofollow" class="btn02"></a><input type="hidden" value="http://twitter.com/share?&text=',obj.slug,'&url=',obj.detailBuyPath,'" id="twitter_val">');
						sharebtns_.push('<a class="btn03"></a><input type="hidden" value="http://www.linkedin.com/shareArticle?mini=true&url=',obj.detailBuyPath,'&title=',obj.slug,'" id="linkedin_val">');
					}
					sharebtns_.push('</div>');
		...
		//	if(format!='METADATA'){
				$.getJSON('https://secure.cctvplus.com/interface/downloadUrl.do?callback=?',{"MID":mid,"DCID":dcid,"type":"News","format":format},function(d_){
					if(d_ && d_.result == 1){
						...
						login_bar_fun();		//ÃÂ¦ÃÂÃÂÃÂ©ÃÂÃÂÃÂ©ÃÂªÃÂÃÂ¨ÃÂ¯ÃÂÃÂ¥ÃÂ¤ÃÂ±ÃÂ¨ÃÂ´ÃÂ¥
						$('#download_box_ li').removeClass('opened');
					}else if(d_ && d_.result == -2){
						if(d_.errCode != ''){
							
							cusmot_alert_fun('Permission validation failed. (error code:'+d_.errCode+')');	//ÃÂ¦ÃÂÃÂÃÂ©ÃÂÃÂÃÂ©ÃÂªÃÂÃÂ¨ÃÂ¯ÃÂÃÂ¥ÃÂ¤ÃÂ±ÃÂ¨ÃÂ´ÃÂ¥
						}else{
							cusmot_alert_fun('Permission validation failed.');	//ÃÂ¦ÃÂÃÂÃÂ©ÃÂÃÂÃÂ©ÃÂªÃÂÃÂ¨ÃÂ¯ÃÂÃÂ¥ÃÂ¤ÃÂ±ÃÂ¨ÃÂ´ÃÂ¥
						}
						...
						if(d_.errCode != ''){
							...
							//cusmot_alert_fun('Get download address failed.');	//ÃÂ¨ÃÂÃÂ·ÃÂ¥ÃÂÃÂÃÂ¤ÃÂ¸ÃÂÃÂ¨ÃÂ½ÃÂ½ÃÂ¥ÃÂÃÂ°ÃÂ¥ÃÂÃÂÃÂ¥ÃÂ¤ÃÂ±ÃÂ¨ÃÂ´ÃÂ¥
						}
	...
	$('#detail_box_').on('click','.buy_box',function(){
		...
		//$.getJSON('http://localhost:8080/GDS_UC/interface/selOrder.do?callback=?',function(d){
			if(d && d.result==1){
	...
	$("#addArchive").click(function(){
			...
			$.ajax({
				...
				success : function (data){
					if(data.result=='-1'){
						...
						msg_.show().html('Order Failed. (error code:'+data.errCode+')');//ÃÂ¦ÃÂ·ÃÂ»ÃÂ¥ÃÂ ÃÂ¨ÃÂ®ÃÂ¢ÃÂ¥ÃÂÃÂÃÂ¥ÃÂ¤ÃÂ±ÃÂ¨ÃÂ´ÃÂ¥
					}
					else if(data.result=='1'){		
						//$('#order_win_msg').show().html('Add order to success');//ÃÂ¦ÃÂ·ÃÂ»ÃÂ¥ÃÂ ÃÂ¨ÃÂ®ÃÂ¢ÃÂ¥ÃÂÃÂÃÂ¦ÃÂÃÂÃÂ¥ÃÂÃÂ
						$.colorbox.close();
					}else if(data.result=='-2'){
							msg_.show().html('Video already exists. (error code:'+data.errCode+')');			//ÃÂ¨ÃÂ®ÃÂ¢ÃÂ¥ÃÂÃÂÃÂ¤ÃÂ¸ÃÂ­ÃÂ¥ÃÂ·ÃÂ²ÃÂ¦ÃÂÃÂÃÂ¦ÃÂ­ÃÂ¤ÃÂ©ÃÂ¡ÃÂ¹
					}else if(data.result=='-3'){	
							msg_.show().html('No choice has been available. (error code:'+data.errCode+')');			//ÃÂ¦ÃÂ²ÃÂ¡ÃÂ¦ÃÂÃÂÃÂ©ÃÂÃÂÃÂ¦ÃÂÃÂ©ÃÂ¥ÃÂ·ÃÂ²ÃÂ¦ÃÂÃÂÃÂ¨ÃÂ®ÃÂ¢ÃÂ¥ÃÂÃÂ
					}
					else if(data.result=='-4'){		
							msg_.show().html('You have just encountered a technical failure (#'+data.errCode+'). Please report it to our customer service representative if possible, thank you.');			//midÃÂ¦ÃÂÃÂÃÂ¨ÃÂÃÂdcidÃÂ¤ÃÂ¸ÃÂºÃÂ§ÃÂ©ÃÂº
					}
					else if(data.result=='-5'){		
							msg_.show().html('You have just encountered a technical failure (#'+data.errCode+'). Please report it to our customer service representative if possible, thank you.');			//ÃÂ¦ÃÂ²ÃÂ¡ÃÂ¦ÃÂÃÂÃÂ¨ÃÂ¿ÃÂÃÂ¤ÃÂ¸ÃÂªÃÂ¨ÃÂ§ÃÂÃÂ©ÃÂ¢ÃÂ
					}
				}
			});
		}
		if($("#cre_new_order_win").is(':checked')){
			...
			$.ajax({
				...
				success : function (data){
					if(data.result=='-1'){
						msg_.show().html('not login');//ÃÂ¦ÃÂÃÂªÃÂ§ÃÂÃÂ»ÃÂ¥ÃÂ½ÃÂ
					}
					else if(data.result=='0'){
							msg_.show().html('Order Failed. (error code:'+data.errCode+')');//ÃÂ¦ÃÂ·ÃÂ»ÃÂ¥ÃÂ ÃÂ¨ÃÂ®ÃÂ¢ÃÂ¥ÃÂÃÂÃÂ¥ÃÂ¤ÃÂ±ÃÂ¨ÃÂ´ÃÂ¥
					}
					else if(data.result=='-2'){
							msg_.show().html('Please specify an order name. (error code:'+errCode+')');			//ÃÂ¨ÃÂ®ÃÂ¢ÃÂ¥ÃÂÃÂÃÂ¥ÃÂÃÂÃÂ§ÃÂ§ÃÂ°ÃÂ¤ÃÂ¸ÃÂºÃÂ§ÃÂ©ÃÂº
					}
					else if(data.result=='1'){
						//$('#order_win_msg').show().html('Add order to success');//ÃÂ¦ÃÂ·ÃÂ»ÃÂ¥ÃÂ ÃÂ¨ÃÂ®ÃÂ¢ÃÂ¥ÃÂÃÂÃÂ¦ÃÂÃÂÃÂ¥ÃÂÃÂ
						$.colorbox.close();
	...
	$('#Locations_data_box input[type="checkbox"]').on('ifChecked ifUnchecked',function(e_){
		...
		if($(this).hasClass('data_all')){//ç¹å»å¨éÂÂ					
...
//	}
	...
	$("#subsearch_val_box_btn").click(function(){
		...
		$.getJSON('http://172.20.1.9/interface/searchRecord.do?callback=?',{"keyword":$.trim($('#subsearch_val_s').val()),"type":searchType},function(d_){
		});
	})
