...
//	}();
	...
	$('#download_box_').on('click','li',function(){	
		...
		if(!this_f.hasClass('opened')){
			...
			if(format!='METADATA'){
				$.getJSON('https://secure.cctvplus.com/interface/downloadUrl.do?callback=?',{"MID":inp_mid,"DCID":inp_dcid,"type":"News","format":format},function(d_){
					if(d_ && d_.result == 1){
						var gncHtml = '<a onclick="addToDQ(\''+dcid+'\',\''+format+'\')" target="_blank" class="download_link">GNC</a>';
						if( d_.cdurl != '' && d_.l3url != '' ){
							d_l_.html('<a href="'+d_.cdurl+'" onclick="downloadUrl(\''+d_.cdurl+'\',\'cdurl\',\''+d_.downloadID+'\')" target="_blank" class="download_link">download1</a><a href="'+d_.l3url+'" onclick="downloadUrl(\''+d_.l3url+'\',\'l3url\',\''+d_.downloadID+'\')" target="_blank" class="download_link">download2</a>'+gncHtml);
						}else if( d_.cdurl != '' && d_.l3url == '' ){
							d_l_.html('<a href="'+d_.cdurl+'" onclick="downloadUrl(\''+d_.cdurl+'\',\'cdurl\',\''+d_.downloadID+'\')" target="_blank" class="download_link">download1</a>'+gncHtml);
						}else if( d_.cdurl == '' && d_.l3url != '' ){
							d_l_.html('<a href="'+d_.l3url+'" onclick="downloadUrl(\''+d_.l3url+'\',\'l3url\',\''+d_.downloadID+'\')" target="_blank" class="download_link">download2</a>'+gncHtml);
						}
...
function addToDQ(dcid, vtype) {
 ...
 $(function(){
		    ...
		    if(null != data.relevantNewList && data.relevantNewList.length>0){
		     ...
		     for(var i = 0;i<data.relevantNewList.length;i++){
                     ...
                     var newcontent = "<div class='same_news_num' style='display:none;' language='"+data.relevantNewList[i].Language+"'>"
					       +"<a href='"+newpath+"'><div class='rele_new' style='text-align:center;margin-top:20px;padding-bottom: 10px;border:1px solid #E9E9E9;width:292px;height:250px;'>"
                           ...
                           +"<span style='font-size:14px;margin-left:-130px;position: absolute;margin-top:15px;'>ID:"+data.relevantNewList[i].DCID+"</span>"
	 ...
	 $("#detail_infotab").on('click','li',function(){
		...
		if(null != newList1 && newList1.length>0){
			var flag1 = false;
			for(var b=0;a<newList1.length;b++){
