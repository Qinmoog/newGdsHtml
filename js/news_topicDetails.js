	...
	var count_num = 1;
	var this_tid_ = Request_url_fun_tid('tid');
//	var this_tid_ = Request_url_fun('tid');
	...
	function Request_url_fun_tid(strName){
	...
	function search_fun_LL(curpage){
			...
			$.getJSON('https://secure.cctvplus.com/searchInterface/topicDetails.do?callback=?',{'pageSize':pagesize,'pageNo':curpage,'tid':this_tid_},function(data){
				if(data && data.title){
...
//			search_list_big.html('<p style="text-align:center; padding:10px;">Loading...</p>');
			$.getJSON('https://secure.cctvplus.com/searchInterface/topicDetails.do?callback=?',
				{
					'pageSize':pagesize,
					'pageNo':curpage,
					'tid':this_tid_
