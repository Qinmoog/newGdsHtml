...
//			console.log('点击');
			...
			var subType2 = strHref.split('/')[4];
			console.log("subType2="+subType2)			
			var isnu = isnumber(subType2);
			console.log("isnu="+isnu)
			var type = 'News';
			if(subType!='news'){
					if(subType == 'exchange' && isnu){
						type = 'News';
					}else{
						type = 'Exchange';
					}
		...
		//		  	poster: img,
			...
			function init_video(uu_){  //小屏
				if(uu_!=undefined&&uu_!=null&&uu_!=''){	
					$('#video_box_s').html('');
					phone_flag = true;
						$('#video_box_s').append('<video webkit-playsinline="true" playsinline="true" id="player1" class="video-js vjs-big-play-centered vjs-fluid"></video>');
