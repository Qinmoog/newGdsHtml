$(function(){
		...
		function search_fun_LL(curpage){
			...
			$.getJSON('https://secure.cctvplus.com/searchInterface/exchange.do?callback=?',
			{
				'pageSize':pagesize,
				'pageNo':curpage,
...
//			video_list_main.html('<p style="text-align:center;font-size:20px;">Loading...</p>');
			$.getJSON('https://secure.cctvplus.com/searchInterface/exchange.do?callback=?',
//			$.getJSON('http://10.0.9.27:8080/searchInterface/exchange.do?callback=?',
				{
					'pageSize':pagesize,
					'pageNo':curpage,
					'channel':channel_code,
			...
			if(!this_f.hasClass('opened')){
				...
				$.getJSON('https://secure.cctvplus.com/interface/downloadUrl.do?callback=?',{"MID":mid,"DCID":dcid,"type":"Exchange","format":format},function(d_){
					if(d_ && d_.result == 1){
						...
						cusmot_alert_fun('No video.(error code:'+d_.errCode+')');	
					}
				});
			}else{
